---
name: skill-create
description: 새로운 Codex skill을 생성합니다. skill 만들기, 명령어 추가 요청 시 사용합니다.
...
새로운 skill을 `.Codex/skills/$name/SKILL.md` 경로에 생성합니다.
...
description: skill 설명          # Codex가 자동 호출 시기를 판단하는 데 사용
...
| `user-invocable` | `false`면 Codex만 호출 가능 (메뉴에서 숨김) |
...
- `disable-model-invocation: false` (기본값) — Codex가 관련 시 자동 로드
